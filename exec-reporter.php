...
/**
 * Handle output, including errors, from programs and scripts in cron jobs
 *
 * Features:
 * - store both STDOUT and STDERR in files
 * - send email notification about errors via SMTP
 *
 * Examples:
 *
 ...
 * `php exec-reporter.php 0 dump.cron.log dump.cron.error.log wget [DASH]O- https://someserver.com/report.php`
 *
 * Arguments are understood as follows:
 * - First: PHP file containing configuration, it must return an array like above (set to '0' to provide configuration through one of the other methods)
 * - Second: File to dump STDOUT in (set to '0' to not dump to file)
...
class exec_reporter {
	...
	public function main() {
		...
		$command = trim(implode(' ', $argv));
		if (!$command) {
			$this->notify_error('Command for Exec-Reporter to execute is missing.', ['Args' => $argv_all]);
		}
		...
		if ($config_file) {
			if (file_exists($config_file)) {
				...
				$this->notify_error('Specified config file for Exec-Reporter does not exist.', ['Args' => $argv_all]);
			}
		}
		...
		if ($this->config['notify_if_stdout'] && $stdout) {
	 ...
	 */
	public function notify_error($message, $parameters) {
		if (!empty($this->config['recipients'])) {
			$this->notify_via_email('Exec-Reporter error: '. $message, $message . PHP_EOL . PHP_EOL . json_encode($parameters, JSON_PRETTY_PRINT|JSON_UNESCAPED_SLASHES));
		} else {
	 ...
	 * Send an email
	 */
	public function notify_via_email($subject, $body, $is_html = false) {
		if (empty($this->config['recipients'])) {
			throw new \Exception('No recipients for Exec-Reporter to send email to.');
		}
			...
			try {
				...
				foreach ($this->config['recipients'] as $key => $recipient) {
					if (is_numeric($key)) {
						...
						$mail->addAddress($key, $recipient);  //email, name
					}
				}
	...
	public function get_safe_config() {
		$config = $this->config;
...
function httpRequest($method, $url, $data = [], $options = []) {
	...
	if ($options['parseJson']) {
		$response = json_decode($rsp, ($options['parseJson'] === 'object' ? false : true));
		if ($response === null) {
...
/*
BELOW THIS POINT IS PHPMailer 6.2.0

The following files from the standard PHPMailer package have simply been copied in here:
- PHPMailer.php
- SMTP.php

The namespace declarations have been removed - no other changes have been made.
*/




/* PHPMailer file: PHPMailer.php */
